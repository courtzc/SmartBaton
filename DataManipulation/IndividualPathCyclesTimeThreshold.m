...
datafiles = ["Session01_ExpA1_003_tXYZ.mat", "Session01_ExpA1_004_tXYZ.mat", "Session01_ExpA1_005_tXYZ.mat", "Session01_ExpA1_006_tXYZ.mat"];

disp(datafiles)

savecycle = zeros(20, 370, 4);
...
for k = 1:length(datafiles)
    ...
    % read in the file
    fprintf(1, 'Now reading %s\n', datafiles(k));
    PosData = load(datafiles(k)).tXYZ;

    %% plot!
    figure
    sgTitleName = sprintf("Path Feature: Normal; Path Habit: Textbook; Threshold: Time; Dataset: %s", strrep(datafiles(k),'_',' '));
    sgtitle(sgTitleName)
    ...
    barTimeLimit = [3.1579, 6.3158, 9.4737, 12.6316, 15.7895, 18.9474, 22.1053, 25.2632, 28.4211, 31.5790, 34.7369, 37.8948];
...
%             sprintf("cycleStartIndex: %d. cycleEndIndex: %d.",cycleStartIndex, cycleEndIndex)
            ...
            fileName = sprintf("ExpA1_00%d_SavedCycle_%d.mat",(k+2), l);
            save(fileName, 'tXYZ');
            l = l+1;
        end
