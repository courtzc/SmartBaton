...
function Read_IMU_And_LeapDevice
    ...
    Motive_filename = sprintf("Data/Session02_ManipulatedData/TrackingDataTime_Resampled_Scaled/Session02_Exp_%s_BBaton_BlanksRemoved_SimpleCentroid_Resampled_Scaled.mat",expID);
...
%     plot3(Motive_Readings((i:i+1),1), Motive_Readings((i:i+1),2), Motive_Readings((i:i+1),3)
    ...
    hold on;
    axis equal;
    titleName = sprintf("Leap Palm Positional Data & Baton Tip Pos & Motive Data from Calibration Exp %s", expID);
    title(titleName)
    view(2)
    ...
    plts{7} = plot3(fig_handle,[0,1],[0,1],[0,1],'color', colourAlt{21}, LineWidth=3);
...
%                     set(plts{7}, 'XData', Motive_Readings(2,:), 'YData', Motive_Readings(3,:), 'ZData',  Motive_Readings(4,:));
                    set(plts{7}, 'XData', motive_array(1,firstNonzeroMotiveArrayIndex:lastNonzeroMotiveArrayIndex), 'YData', motive_array(2,firstNonzeroMotiveArrayIndex:lastNonzeroMotiveArrayIndex), 'ZData',  motive_array(3,firstNonzeroMotiveArrayIndex:lastNonzeroMotiveArrayIndex));
                    drawnow
                end
            end
